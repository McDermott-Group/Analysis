...
file_length = 400 * 5
t = Q4_NoPoison.time_data[:file_length]*5/3.7
...
Q4_list = [Q4_NoPoison, Q4_Neg20dBm, Q4_Neg6dBm]
...
for i, Q4 in enumerate(Q4_list):
    weights = np.ones_like(Q4.one_state_data) /float(len(Q4.one_state_data))
    # print('Q4.one_state_data=', Q4.one_state_data)
    ax[i][1].hist(Q4.one_state_data, bins=bin_list, weights=weights,
                  label=Q4.label)
    # ax[i][1].hist(Q4.one_state_data, bins=bin_list,
    #               label=Q4.label)
    ax[i][1].legend(loc=1)
    ax[i][1].grid()

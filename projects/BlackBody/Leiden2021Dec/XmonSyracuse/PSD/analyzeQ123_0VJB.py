...
for QB_id in QBs:
    ...
    for J1Bias in get_bias_list(QB_id):
        experiment_name_PSD = (QB_id+'_PSD_'+str(J1Bias)+'uDACJ1')
        print(experiment_name_PSD)
        PSD_file_Number = np.arange(0, 25, 1)
        PSD_file = [QP_path.format(date+'_'+QB_id+'_PSD', experiment_name_PSD, experiment_name_PSD) + '_{:03d}.mat'.format(i) for i in PSD_file_Number]
        QPT_Q = QPTunneling_Harrison(name='{} with J1 = {} uDAC, {}GHz'.
        ...
        avg_fidelity, avg_parity, parity_uncertainty =plotFittedPSD_Harrison(QPT_Q, save=False, name='Zoomed Optimized {} with J1 ={} uDAC {}GHz'.
                                  format(QB_id, str(J1Bias), str(J1Bias*4.8)),excluded_points=ep, concatenate_records=cr)#, ylim=ylim)

        cr=get_cr(avg_parity)
        ep=2

        avg_fidelity, avg_parity, parity_uncertainty = plotFittedPSD_Harrison(QPT_Q, save=True,
                                                                              name='Zoomed Optimized {} with J1 ={} uDAC {}GHz'.

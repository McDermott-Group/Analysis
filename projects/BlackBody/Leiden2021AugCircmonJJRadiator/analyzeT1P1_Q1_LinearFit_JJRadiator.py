...
J2Bs=sorted(list(np.arange(35000,42000,500))+list(np.arange(42500,63000,500)))
# J2Bs = (list(np.arange(0,101,10)) + list(np.arange(35, 56, 2))) #August18
# J2Bs = list(np.arange(100,980,20)) #August17
# suffix = 'mDACJ2'
suffix = 'uDACJ2'
for it,J2B in enumerate(J2Bs):
    print(it)
    experiment_name_P1 = experiment_name_P1_global+ str(J2B) + suffix
    experiment_name_T1 = experiment_name_T1_global + str(J2B) + suffix
...
plt.title('Q1')
plt.plot(J2Bs, GammaUp_list, 'o-', label='Gamma Up')
plt.xlabel('JJ Bias ('+suffix+')')

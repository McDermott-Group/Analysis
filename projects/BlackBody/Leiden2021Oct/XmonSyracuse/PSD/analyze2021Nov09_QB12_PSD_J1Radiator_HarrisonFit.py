...
def get_bias_list(QB_id):
    if QB_id == 'Q1':
        return [int(bias*1000) for bias in (list(np.arange(0,25,5)))]
    elif QB_id == 'Q2':
        #return [200000]#for debug
        return [int(bias*1000) for bias in (list(np.arange(0.180,0.220,0.001))+list(np.arange(0.240,0.340,0.001))+list(np.arange(0.470,0.530,0.001))+list(np.arange(0.600,0.650,0.001)))]
    else:
        return [int(bias*1000) for bias in (list(np.arange(0.240,0.319,0.001)))]
...
for QB_id in QBs:
    ...
    for J1Bias in get_bias_list(QB_id):
        experiment_name_PSD = (QB_id+'_PSD_'+str(J1Bias)+'mVJ1_HighDensity')

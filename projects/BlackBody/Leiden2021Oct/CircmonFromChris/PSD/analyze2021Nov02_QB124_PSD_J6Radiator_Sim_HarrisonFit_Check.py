...
def get_bias_list(QB_id):
    #return [int(bias*1000) for bias in (list(np.arange(0.0,0.05,0.01))+list(np.arange(0.06,0.585,0.005)))]
    if QB_id == 'Q1':
        return [int(bias * 1000) for bias in (list(np.arange(0.1, 0.23, 0.005))+list(np.arange(0.4, 0.585, 0.005)))]
    # return [int(bias * 1000) for bias in (list(np.arange(0.48, 0.585, 0.005)))]
...
for QB_id in QBs:
    ...
    for J6Bias in get_bias_list(QB_id):
        experiment_name_PSD = (QB_id+'_PSD_'+str(J6Bias)+'mVJ6')

...
def get_bias_list(QB_id):
    if QB_id == 'Q1':
        return [int(bias) for bias in (list(np.arange(100,113,1))+list(np.arange(114,160,1)))]
    elif QB_id == 'Q2':
        return [int(bias) for bias in (list(np.arange(100,160,1))+list(np.arange(370,420,1)))]
    return [int(bias) for bias in (list(np.arange(190,260,1)))]
...
for QB_id in QBs:
    ...
    for J6Bias in get_bias_list(QB_id):
        ...
        QPT_Q = QPTunneling_Harrison(name='{} with J6 = {} mV'.
        ...
        avg_fidelity, avg_parity, parity_uncertainty =plotFittedPSD_Harrison(QPT_Q, save=False, name='Zoomed Optimized {} with J6 ={} mV'.
                                  format(QB_id, str(J6Bias)),excluded_points=ep, concatenate_records=cr)#, ylim=ylim)

        cr=get_cr(avg_parity)
        ep=2

        avg_fidelity, avg_parity, parity_uncertainty = plotFittedPSD_Harrison(QPT_Q, save=True,
                                                                              name='Zoomed Optimized {} with J6 ={} mV'.
                                                                              format(QB_id, str(J6Bias)),
                                                                              excluded_points=ep,

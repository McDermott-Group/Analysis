...
J2Biaslist1= list(np.arange(-40, 280, 20))
J2Biaslist2= list(np.arange(300,700,20))+list(np.arange(-310,290,20))#list(np.arange(450,690,20))+list(np.arange(-305,55,20))#list(np.arange(480, 681, 20))+ list(np.arange(-310, 11, 20))
start_file = 0
num_files = 50
# J2Biaslist = [40, 42, 44]
fparity = {}
...
for QB_id in QBs:
    ...
    for J2Bias in J2Biaslist1:
        ...
        experiment_name_PSD = ('PSD_{}mDAC_JB2_Q{}_HighRO_Retuned'.format(str(J2Bias),str(1)))
        ...
        avg_fidelity, avg_parity, parity_uncertainty =plotFittedPSD_Harrison(QPT_Q, save=True, name='{} with J2 ={} mDAC'.
                                  format(QB_id, str(J2Bias)),excluded_points=ep,concatenate_records=cr,ylim=[10 ** (-5), 10 ** (-3)],number=9)
        # print(QB_id)

...
expt_paths =  [base_path + [user] + [device_name] + [d] for d in dates]#+ [experiment_base_name.replace(" ", "_")] for d in dates]
paths = [os.path.join(*([r'Z:\mcdermott-group\data'] + expt_path)) for expt_path in expt_paths]

#Idea here:
#Loop over qubits:
#   Read in SINGLE IQ Calibration data and create calibration object
#   Loop over temperature:
#       Loop over blobs in T1_Blobs_BB folder:
#           Calibrate and extract P1; save P1 and idle time
#       Fit T1, P1 from above lists. Save gamma up and temperature.
...
for qb_id in [1,2]:
    ...
    for i in range(len(paths)):
        ...
        for dataSet in os.listdir(path):
            #this gets all the temperatures
            parsed = parse.parse('T1_BB_Blobs_Q{:d}_{:d}mK_BB', dataSet)
            if parsed is not None:
                if parsed[0] == qb_id:
                    pass
                    #open the folder corresponding to T=parsed[1] then loop blobs

...
elseif exist('OFF','var')
    if OFF == 0
        [~,~,dat]=xlsread('Z:\mcdermott-group\users\Naveen\Projects\Dissipator\EXCEL\March 2018\GammaPhiVsNbars.xlsx', 'Dissipator ON')
    else
        [~,~,dat]=xlsread('Z:\mcdermott-group\users\Naveen\Projects\Dissipator\EXCEL\March 2018\GammaPhiVsNbars.xlsx', 'Dissipator OFF')   
    end
    nbar1 = dat(a:b,n_col)
...
std_Slope_confint = (Slope_confint(2,2) - Slope_confint(1,2))/2;
...
if fitresult.p2 < 0
    ...
    Equation2 = ['Slope  = ','$ \pm',num2str(std_Slope_confint), '$'];   
    Equation3 = ['Intercept  = ','$ \pm',num2str(std_GammaPhi_confint), '$'];    

function CircleFitResult = FitMeasData2Circle(XY)
%--------------------------------------------------------------------------
%
...
for iter=1:IterMax
    ...
    if (abs(ynew)>abs(yold))
        disp('Algorithm goes wrong direction: |ynew| > |yold|');
        xnew = 0;
        break;
    end
    ...
    if (xnew<0.)
        fprintf(1,'Algorithm negative root:  x=%f\n',xnew);
        xnew = 0;
    end
end
...
CircleFitResult = [Center+centroid , sqrt(Center*Center'+Mz+2*xnew)];
end   
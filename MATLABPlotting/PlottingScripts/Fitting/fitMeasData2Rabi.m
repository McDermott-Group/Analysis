function fitMeasData2Rabi(data_variable, data)
...
if length(dep_rels) == 1
    ...
    Tcoh = max([abs(ci(1, 5) - f.T), abs(ci(2, 5) - f.T)]);
    Tcstr = ['T_{coh} = ', num2str(f.T, 4), ' ± ',...
        num2str(Tcoh, 4), xunits];
    
    TRabi = max([abs(ci(1, 3) - f.d), abs(ci(2, 3) - f.d)]);
    TRstr = ['T_{Rabi} = ', num2str(f.d, '%.1f'), ' ± ',...
            num2str(TRabi, '%.1f'), xunits];
...
elseif length(dep_rels) == 2 % Plot 2D data.
    ...
    parfor k = 1:length(indep_vals1)
        f = RabiFit(indep_vals2, dep_vals(k, :)');
        ci = confint(f);
...
function f = RabiFit(x, y)
    ft = fittype(['a + b * exp(-(x / T) ) * ',...
        'sin(2 * pi * (x / d) + e)'],...
        'independent', 'x', 'dependent', 'y',...
        'coefficients', {'a', 'b', 'd', 'e', 'T'});
    ...
    opts.Lower = [-max(abs(y)), -max(abs(y)), 0, -pi,...
                   max(x) / 100];
%     opts.StartPoint = [y(1), max(y) - y(1), max(x) / 2, max(x) / 20,...
%                        0, 10 * max(x)];
    opts.StartPoint = [mean(y), max(y) - min(y), max(x) / 2,...
                       0, 5 * max(x)];
    opts.Upper = [max(abs(y)), max(abs(y)), 100 * max(x),...
                   pi, 1000 * max(x)];
    f = fit(x, y, ft, opts);
end
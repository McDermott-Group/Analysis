function plotRescaled2DMeasData(normalization_direction)
%plotMinNorm2DMeasData(NORMALIZATION_DIRECTION) Plot a line-by-line
%rescaled 2D data. NORMALIZATION_DIRECTION should be
...
for data_index = 1:length(data.dep)
    ...
    if length(dep_rels) == 2
        ...
        if ~isempty(strfind(dep_name, 'Probability')) ||...
            ...
            extra_title = 'Line-by-Line Rescaled ';
            extra_filename = ['_rescaled_', normalization_direction];
        else
            extra_title = '';
            extra_filename = '';
        end

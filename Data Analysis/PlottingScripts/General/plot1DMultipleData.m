...
for data_index = 1:length(data(1).dep)
    ...
    if length(data(1).rels.(dep_name)) == 1
        ...
        if length(data(k).rels.(dep_name)) ~= 1 || ~strcmp(indep_name, data(k).rels.(dep_name){1})
            error('The selected files do not match.')
        end
        ...
        if isempty(legend_options)
            error('At least two files have indistinguishable sets of parameters. Is is likely that the same file was selected more than once.')
        end
        legend_options_whitespaces = cellfun(@(str) strrep(str, '_', ' '), legend_options, 'UniformOutput', false);
        ...
        xmin = min(reshape([data(:).(indep_name)], [], 1));
        xmax = max(reshape([data(:).(indep_name)], [], 1));
        if xmax == xmin
            xmax = xmax + eps;
        end
        ...
        if errorbar_flag % Plot an errobar graph.
            ...
            hold on
            for k = 1:length(filenames)
                errorbar(data(k).(indep_name), data(k).(dep_name), data(k).error.(dep_name),...
    ...
    if length(data(1).rels.(dep_name)) > 1 && print_messages
        disp(['Data variable ''', strrep(dep_name, '_', ' '), ''' depends on more than one sweep variable. ',...

...
r_direct_no_tr = 6e-6; % in units of 1 / \tau_0 %(assuming n_{qp} in units of n_{cp})
r_phonon_no_tr = .05; % in units of 1 / \tau_0 %(assuming n_{qp} in units of n_{cp})
c_no_tr = 0.005; % trapping rate in units of 1 / \tau_0

r_direct_tr = 3e-6; % 1.61 * r_direct_no_tr; % in units of 1 / \tau_0 %(assuming n_{qp} in units of n_{cp})
r_phonon_tr = .05; % in units of 1 / \tau_0 %(assuming n_{qp} in units of n_{cp})
...
for k = 1:length(V_no_tr)
    if V_no_tr(k) > 1
        [~, ~, ~, ~, nqp, ~, P_sim_no_tr(k)] = twoStageQuasi0DModel(Tph, tspan,...
            V_no_tr(k), r_direct_no_tr, r_phonon_no_tr, c_no_tr, false);
...
for k = 1:length(V_tr)
    if V_tr(k) > 1
        [~, ~, ~, ~, nqp, ~, P_sim_tr(k)] = twoStageQuasi0DModel(Tph, tspan,...
            V_tr(k), r_direct_tr, r_phonon_tr, c_tr, false);

...
r = 4e-9; % in units of 1 / \tau_0 %(assuming n_{qp} in units of n_{cp})
c = .5e-3; % trapping rate in units of 1 / \tau_0
...
for k = 1:length(V)
    ...
    [t, ~, ~, ~, n_qp, ~, ~, P(k)] = pairBreakingTrapping0DModel(r, c, V(k), Tph, tspan);

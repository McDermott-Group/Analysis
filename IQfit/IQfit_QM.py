...
class IQ_fit(object):
    ...
    def IQ_fit_single_power(self, file_index=1, power=None, Qc_guess=None, Qi_guess=None, att=0, save=False,
                            Singleshotlike=False, CavitySpectroscopylike=False, ind=1,
                            auto_res_freq=True):  # index of the file to be analyzed from the bottom (index starts from 1)
        d = dataChest(self.path)
        dataset_name = d.ls()[0][-1 * file_index]

        #if "SingleShot" in dataset_name or Singleshotlike is True:
        if "Resonator_Spectroscopy" in dataset_name or Singleshotlike is True:
            d.openDataset(dataset_name)

            dep_variables = d.getVariables()[1]
            for ii in range(0, len(dep_variables)):
                var_name = str(dep_variables[ii][0])
            ...
            #phase = np.unwrap(2 * np.asarray(d.getData()[:, 1])[ind:-ind] * 2. * np.pi / 360.) / 2
            phase = np.unwrap(2*np.asarray(d.getData()[:, 4])[ind:-ind])/2

            phase = sig.detrend(phase)
            phase = phase - phase.mean()
            phase = phase *-1
            #S_21 = 10 ** (S_21_dB / 20)
            Amp = np.asarray(d.getData()[:, 3])[ind:-ind]
            S_21 = Amp**2
            S_21_dB = 20*np.log10(S_21)
            ...
            if auto_res_freq is True:
                ...
                min_frequency = f[np.argmin(Amp, axis=0)]
                print("I am here!")
            else:
            ...
            if Qc_guess is None:
                ...
                if Qi_guess is None:
                    ...
                    def func(params, f, np_func):
                        ...
                        dx = (f - f_0) / f_0
                        return np_func(
                            A * (1.0 + (Q_i / Q_c) * np.exp(1j * phi_0) * (1. / (1. + 2. * 1j * Q_i * dx))) * np.exp(
                                -1j * (dx * f_0 * phi_v + phi)))  # (f-f_0)

                    def objective(params):
                        resid = np.sqrt((np.real(1. / S_21) - func(params, f, np.real)) ** 2 + (
                                    np.imag(1. / S_21) - func(params, f, np.imag)) ** 2)
                        return resid

                    df = f[-1] - f[0]
                    phi_v_guess = ((phase[-1] - phase[0]) / df)  # *min_frequency
                    print("phi_v_guess=", phi_v_guess)
                    alpha_guess = (np.mean(np.absolute(S_21)[-20:-1]) - np.mean(np.absolute(S_21)[0:20])) / (
                                np.mean(f[-20:-1]) - np.mean(f[0:20]))
                    print("alpha_guess=", alpha_guess)
                    phi_guess = phase[np.argmin(S_21_dB, axis=0)]
                    # phi_guess = phase[np.argmin(Amp, axis=0)]
                    print("phi_guess=", phi_guess)

                    # create a set of Parameters
                    params = Parameters()
                    params.add('A', value=1.0, min=.1, max=1.7)
                    params.add('Q_i', value=Qi_guess, min=1000, max=9000)
                    # params.add('Q_i', value= Q_i_guess, min=0.80*Q_i_guess, max=1.2*Q_i_guess)
                    params.add('Q_c', value=Qc_guess, min=1000, max=2000)
                    params.add('f_0', value=min_frequency, min=min_frequency - df / 5., max=min_frequency + df / 5.)
                    params.add('phi_v', value=phi_v_guess)
                    # params.add('phi_v', value=phi_v_guess, min=1000. * phi_v_guess, max=phi_v_guess * .001)
                    params.add('phi_0', value=0, min=-np.pi, max=np.pi)
                    params.add('phi', value=phi_guess, min=-np.pi, max=np.pi)
                    ...
                    if save is True:
                        ...
                        with open(fname, "a") as fn:
                            np.savetxt(fn, np.c_[(power - att, params['Q_i'].value, params['Q_c'].value)], fmt="%10.5e",
                                       delimiter=' ', header="power        Q_i        Q_c")
                    ...
                    plt.plot(f / 1e9,
                             360. * np.angle(func(params, f, np.real) + 1j * func(params, f, np.imag)) / (2.0 * np.pi))
                    plt.title('<%s' % s_param_type)
                    plt.xlabel('Frequency (GHz)')
                    ...
                    params.add('Q_c', value=Qc_guess, min=1e2, max=9e6)
                    ...
                    if save is True:
                        fname = "IQ_data" + str(self.res_freq) + ".txt"
                        with open(fname, "a") as fn:
                            np.savetxt(fn, np.c_[(power - att, Q_i, params['Q_c'].value)], fmt="%10.5e", delimiter=' ',
                                       header="power        Q_i        Q_c")
                ...
                if Qi_guess is None:
                    ...
                    params = Parameters()
                    params.add('A', value=1.0, min=.1, max=1.5)
                    params.add('Q_i', value=Qi_guess, min=10, max=90000)
                    params.add('f_0', value=min_frequency, min=min_frequency - df / 20., max=min_frequency + df / 20.)
                    params.add('phi_v', value=phi_v_guess, min=1000. * phi_v_guess, max=phi_v_guess * .001)
                    params.add('phi_0', value=0, min=-2 * np.pi, max=2 * np.pi)
                    params.add('phi', value=phi_guess, min=-2 * np.pi, max=2 * np.pi)
                    ...
                    if save is True:
                        fname = "IQ_data" + str(self.res_freq) + ".txt"
                        with open(fname, "a") as fn:
                            np.savetxt(fn, np.c_[(power - att, params['Q_i'].value, Q_c)], fmt="%10.5e", delimiter=' ',
                                       header="power        Q_i        Q_c")
                    ...
                    if save is True:
                        fname = "IQ_data" + str(self.res_freq) + ".txt"
                        with open(fname, "a") as fn:
                            np.savetxt(fn, np.c_[(power - att, Q_i, Q_c)], fmt="%10.5e", delimiter=' ',
                                       header="power        Q_i        Q_c")
        ...
        if "CavitySpectroscopy" in dataset_name or CavitySpectroscopylike is True:
            ...
            phase = np.asarray(data[first_index:last_index, 2] * 2. * np.pi / 360.)[ind:-ind]
            ...
            if Qc_guess is None:
                ...
                if Qi_guess is None:
                    ...
                    params = Parameters()
                    params.add('A', value=1.0, min=.1, max=1.5)
                    params.add('Q_i', value=Qi_guess, min=0.5e3, max=9e6)
                    # params.add('Q_i', value= Q_i_guess, min=0.80*Q_i_guess, max=1.2*Q_i_guess)
                    params.add('Q_c', value=Qc_guess, min=1e2, max=9e6)
                    params.add('f_0', value=min_frequency, min=min_frequency - df / 20., max=min_frequency + df / 20.)
                    params.add('phi_v', value=phi_v_guess, min=1000. * phi_v_guess, max=phi_v_guess * .001)
    ...
    def IQ_fit_multi_power(self, file_index=1, powers=None, Qc_guess=None, Qi_guess=None, att=0, save=False, ind=1,
                           auto_res_freq=True):
        d = dataChest(self.path)
        dataset_name = d.ls()[0][-1 * file_index]
            ...
            if Qi_guess is None:
                ...
                for index, power in enumerate(powers):
                    ...
                    if save is True:
                        ...
                        with open(fname, "a") as f:
                            if index == 0:
                                np.savetxt(f, np.c_[(
                                power - att, params['Q_i'].value, params['Q_c'].value, params['Q_i'].stderr,
                                params['Q_c'].stderr)], fmt="%10.5e", delimiter=' ',
                                           header="power        Q_i        Q_c        Q_i_error        Q_c_error")
                            else:
                                np.savetxt(f, np.c_[(
                                power - att, params['Q_i'].value, params['Q_c'].value, params['Q_i'].stderr,
                                params['Q_c'].stderr)], fmt="%10.5e", delimiter=' ')
                ...
                for index, power in enumerate(powers):
                    ...
                    if save is True:
                        ...
                        with open(fname, "a") as f:
                            if index == 0:
                                np.savetxt(f, np.c_[(power - att, Q_i, params['Q_c'].value, params['Q_c'].stderr)],
                                           fmt="%10.5e", delimiter=' ',
                                           header="power        Q_i        Q_c        Q_c_error")
                            else:
                                np.savetxt(f, np.c_[(power - att, Q_i, params['Q_c'].value, params['Q_c'].stderr)],
                                           fmt="%10.5e", delimiter=' ')
            ...
            if Qi_guess is None:
                ...
                for index, power in enumerate(powers):
                    ...
                    if save is True:
                        ...
                        with open(fname, "a") as f:
                            if index == 0:
                                np.savetxt(f, np.c_[(power - att, params['Q_i'].value, Q_c, params['Q_i'].stderr)],
                                           fmt="%10.5e", delimiter=' ',
                                           header="power        Q_i        Q_c        Q_i_error")
                            else:
                                np.savetxt(f, np.c_[(power - att, params['Q_i'].value, Q_c, params['Q_i'].stderr)],
                                           fmt="%10.5e", delimiter=' ')
